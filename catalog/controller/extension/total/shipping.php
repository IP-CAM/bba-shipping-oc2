...
class ControllerExtensionTotalShipping extends Controller {
	public function index() {
		if ($this->config->get('shipping_status') && $this->config->get('shipping_estimator') && $this->cart->hasShipping()) {
			...
			//frd
			//$data['country_id'] = 1;
			if (isset($this->session->data['shipping_address']['bbacountry'])) {
				$data['bbacountry'] = $this->session->data['shipping_address']['bbacountry'];
			} else {
				$data['bbacountry'] = 'AU';
			}
			if (isset($this->session->data['shipping_address']['bbapostcode'])) {
				$data['bbapostcode'] = $this->session->data['shipping_address']['bbapostcode'];
			} else {
				$data['bbapostcode'] = '';
			}

			if (isset($this->session->data['shipping_address']['bbacity'])) {
				$data['bbacity'] = $this->session->data['shipping_address']['bbacity'];
			} else {
				$data['bbacity'] = '';
			}
			//--
